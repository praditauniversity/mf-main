...
const AddModalDailyReport = () => {
    ...
    const handleFormChangeEqupment = (value, index) => {
        const dataEquip = equipment.map((objItem, objIndex) => {
            return objIndex === index ? value : objItem
        })
        setProjectObjectives(dataEquip)

        console.log("DATA", dataEquip)
        console.log("PROJECTEQUP", equipment)
    }
                            ...
                            <div className="flex min-h-full items-center justify-center p-4 text-center">
                                ...
                                >
                                    <Dialog.Panel className="px-24 py-16 w-full max-w-5xl transform overflow-hidden rounded-lg bg-white p-6 text-left align-middle shadow-xl transition-all">
                                        ...
                                        {/* Equipment */}
                                        {/* <div className="pb-2">
                    <label className="block uppercase tracking-wide text-darkest text-xs font-bold mb-2">Equipment</label>
                                ...
                                <div className="pb-2 w-full min-w-5xl" id="buttonInside">
                                    <div className="flex justify-start">
                                        <input
                                            ...
                                            value={input}
                                            onChange={event => handleFormChangeEqupment(event.target.value, index)}
                                        />
                                        {equipment.length !== 1 && <button className="bg-primary hover:bg-primary-800 py-2.5 px-2.5 rounded-lg ml-2" onClick={() => removeFieldsEquipment(index)}><IconDeleteForm /></button>}
                                        {equipment.length - 1 === index && <button className="bg-primary hover:bg-primary-800 py-2.5 px-2.5 rounded-lg ml-2" onClick={() => { setEquipment([...equipment, '']) }}><IconPlusForm /></button>}
                                    </div>
                                </div>
                            </div>
                        )
